...
class CodingPanel {
  ...
  private fetchChildren(metadataType) {
    ...
    if (!node.inFolder) {
      vscode.window.withProgress(
        ...
        (progress, token) => {
          ...
          var p = new Promise((resolve) => {
            let sfdxCmd =
              "sfdx force:mdapi:listmetadata -a " +
              this.VERSION_NUM +
              " --json -m " +
              mType;
      ...
      let sfdxCmd =
        "sfdx force:mdapi:listmetadata --json -a " +
        this.VERSION_NUM +
        " -m " +
        folderType;
      ...
      (progress, token) => {
        ...
        var p = new Promise((resolve) => {
          var foo: child.ChildProcess = child.exec(
            "sfdx force:mdapi:describemetadata -a " +
              this.VERSION_NUM +
              " --json",
            {
              maxBuffer: 1024 * 1024 * 6,
  ...
  private _getHtmlForWebview() {
    ...
    const scriptPathOnDisk = vscode.Uri.file(
      path.join(this._extensionPath, "client", "build", scriptEntryPoints[0])
    );
    const scriptUri = scriptPathOnDisk.with({ scheme: "vscode-resource" });

    const runtimeScriptPathOnDisk = vscode.Uri.file(
      path.join(this._extensionPath, "client", "build", scriptEntryPoints[1])
    );
    const runtimeScriptUri = runtimeScriptPathOnDisk.with({
      scheme: "vscode-resource"
    });

    const staticScriptPathOnDisk = vscode.Uri.file(
      path.join(this._extensionPath, "client", "build", scriptEntryPoints[2])
    );
    const staticScriptUri = staticScriptPathOnDisk.with({
      scheme: "vscode-resource"
    });

    const stylePathOnDisk = vscode.Uri.file(
      path.join(this._extensionPath, "client", "build", styleEntryPoints[0])
    );
